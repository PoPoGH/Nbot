...
module.exports = {
    ...
    run: async (client, message, args, prefix) => {
        try {
            ...
            if (!user || user == null || user.id == null || !user.id) return message.reply("Membre non trouver");
            ...
            embeduserinfo.setAuthor("Informations sur:   " + member.user.username + "#" + member.user.discriminator, member.user.displayAvatarURL({ dynamic: true }),)
            embeduserinfo.addField('**❱ Pseudo:**', `<@${member.user.id}>\n\`${member.user.tag}\``, true)
            ...
            embeduserinfo.addField('**❱ Date de création du compte:**', "\`" + moment(member.user.createdTimestamp).format("DD/MM/YYYY") + "\`\n" + "`" + moment(member.user.createdTimestamp).format("hh:mm:ss") + "\`", true)
            embeduserinfo.addField('**❱ Membre du serveur depuis:**', "\`" + moment(member.joinedTimestamp).format("DD/MM/YYYY") + "\`\n" + "`" + moment(member.joinedTimestamp).format("hh:mm:ss") + "\`", true)
            ...
            embeduserinfo.addField('**❱ Plus haut role:**', `${member.roles.highest.id === message.guild.id ? 'None' : member.roles.highest}`, true)
            embeduserinfo.addField('**❱ Bot:**', `\`${member.user.bot ? "✔️" : "❌"}\``, true)
